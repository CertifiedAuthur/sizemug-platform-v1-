async function getPopularList() {
  ...
  const popular = data.results.map((user) => {
    return {
  ...
  return renderPopularReq(popular);
}
getPopularList();

function renderPopularReq(populars) {
  ...
  populars.forEach((popular, i) => {
    const html = `
                        <div class="activity_row--item ${
                          i % 2 === 0 ? "active" : ""
                        }">
                          <div class="photo">
                           <img src="${popular.avatar}" alt="${popular.name}" />
                          </div>
                          <div>
                            <!-- prettier-ignore -->
                            <svg
                              ...
                              >
                                ...
                                <path
                                  fill="#ffff"
                                  d="M13.656 10.451c.896.658 1.344.987 1.344 1.55c0 .562-.448.89-1.344 1.548a12.83 12.83 0 0 1-.718.495c-.197.125-.421.254-.653.381c-.894.49-1.34.734-1.741.463c-.401-.27-.437-.838-.51-1.971c-.02-.32-.034-.635-.034-.917c0-.282.013-.596.034-.917c.072-1.133.109-1.7.51-1.97c.4-.271.847-.027 1.74.462c.233.127.457.256.654.381c.226.143.471.314.718.495"
                                />
                              </svg>
