...
//////////////////////////////////////////////////
// check buton mode
function checkoutBtnMode() {
  checkoutBtn.disabled = allShippingItems.length === 0; // just add the disabled property on the element
}
checkoutBtnMode();
...
checkoutBtn.addEventListener("click", () => {
  choosePaymentOption.classList.remove(HIDDEN); //show payment method options
  cartSlider.classList.remove("active"); // hide cart list container
});
...
choosePaymentOption.addEventListener("click", function (e) {
  ...
  if (next) {
    ...
    if (gateWay === "coin") {
      ...
      renderShippingItem(shippingItemsEl);
    }
