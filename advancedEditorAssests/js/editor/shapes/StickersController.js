    ...
    function ($scope, $rootScope, canvas, simpleShapes, history, settings) {
      ...
      $scope.addToCanvas = function (category, index, e) {
        ...
        if (category.type === "svg") {
          fabric.loadSVGFromURL("advancedEditorAssests/images/stickers/" + category.name + "/" + index + "." + category.type, function (objects, options) {
          ...
          fabric.util.loadImage("advancedEditorAssests/images/stickers/" + category.name + "/" + index + "." + category.type, function (img) {
            var image = new fabric.Image(img);
            image.name = "sticker";
            canvas.fabric.add(image);
            image.center();
