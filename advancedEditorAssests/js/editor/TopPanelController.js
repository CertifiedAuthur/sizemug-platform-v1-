...
angular
  ...
  .controller("TopPanelController", [
    ...
    "history",
    "saver",
    function ($rootScope, $scope, $mdDialog, $mdToast, $$rAF, canvas, history, saver) {
      ...
      $scope.transformToClickElement = function (panel, e) {
        ...
        panel.css("transform", "translate3d(" + (-panelRect.left + clickRect.left + clickRect.width / 2 - panelRect.width / 2) + "px," + (-panelRect.top + clickRect.top + clickRect.height / 2 - panelRect.height / 2) + "px," + "0) scale(" + scaleX + "," + scaleY + ")");
      };
      ...
      $scope.openSampleImage = function () {
        canvas.loadMainImage("advancedEditorAssests/images/lotus.jpg");
        $scope.closeUploadDialog();
        $rootScope.started = true;
      };
