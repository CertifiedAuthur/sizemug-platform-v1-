...
(function (window, angular, undefined) {
  ...
  (function () {
     ...
     * requirements.
     */
    angular.module("material.core", ["material.core.gestures", "material.core.theming"]).config(MdCoreConfigure);
 ...
 */
      ...
      function dialogDefaultOptions($mdAria, $document, $mdUtil, $mdConstant, $mdTheming, $mdDialog, $timeout, $rootElement, $animate, $$rAF, $q) {
        ...
        function onShow(scope, element, options) {
          ...
          if (options.hasBackdrop) {
            // Fix for IE 10
            var computeFrom = options.parent[0] == $document[0].body && $document[0].documentElement && $document[0].documentElement.scrollTop ? angular.element($document[0].documentElement) : options.parent;
  ...
  (function () {
    ...
    (function () {
      ...
      angular.module("material.components.fabSpeedDial", ["material.core", "material.components.fabTrigger", "material.components.fabActions"]).directive("mdFabSpeedDial", MdFabSpeedDialDirective).animation(".md-fling", MdFabSpeedDialFlingAnimation).animation(".md-scale", MdFabSpeedDialScaleAnimation);
  ...
  (function () {
    ...
    (function () {
      ...
      angular.module("material.components.fabToolbar", ["material.core", "material.components.fabTrigger", "material.components.fabActions"]).directive("mdFabToolbar", MdFabToolbarDirective).animation(".md-fab-toolbar", MdFabToolbarAnimation);
              ...
              return {
                ...
                tiles: tilePositions.map(function (ps, i) {
                  return {
                    ...
                    style: getTileStyle(ps.position, ps.spans, props.colCount, props.rowCount, props.gutter, props.rowMode, props.rowHeight),
                  };
                }),
              };
            })
  ...
  (function () {
    ...
    angular.module("material.components.menu", ["material.core", "material.components.backdrop"]).directive("mdMenu", MenuDirective).controller("mdMenuCtrl", MenuController);
    ...
    function MenuProvider($$interimElementProvider) {
      ...
      function menuDefaultOptions($$rAF, $window, $mdUtil, $mdTheming, $timeout, $mdConstant, $document) {
        ...
        function positionMenu(el, opts) {
          ...
          if (!opts.alreadyOpen) {
            containerNode.style[$mdConstant.CSS.TRANSFORM] = "scale(" + Math.min(originNodeRect.width / containerNode.offsetWidth, 1.0) + "," + Math.min(originNodeRect.height / containerNode.offsetHeight, 1.0) + ")";
          }
...
***************************************************/
      ...
      function selectDefaultOptions($mdSelect, $mdConstant, $$rAF, $mdUtil, $mdTheming, $timeout, $window) {
        ...
        function animateSelect(scope, element, opts) {
          ...
          if (shouldOpenAroundTarget) {
            ...
            top = Math.floor(targetRect.top + targetRect.height / 2 - centeredRect.height / 2 - centeredRect.top + contentNode.scrollTop);
        ...
        function positionTooltip() {
          ...
          function positionBackground() {
            var size = direction === "left" || direction === "right" ? Math.sqrt(Math.pow(tipRect.width, 2) + Math.pow(tipRect.height / 2, 2)) * 2 : Math.sqrt(Math.pow(tipRect.width / 2, 2) + Math.pow(tipRect.height, 2)) * 2,
              position = direction === "left" ? { left: 100, top: 50 } : direction === "right" ? { left: 0, top: 50 } : direction === "top" ? { left: 50, top: 100 } : { left: 50, top: 0 };
            background.css({
              width: size + "px",
              height: size + "px",
              left: position.left + "%",
          ...
          function getPosition(dir) {
            return dir === "left"
              ? { left: parentRect.left - tipRect.width - TOOLTIP_WINDOW_EDGE_SPACE, top: parentRect.top + parentRect.height / 2 - tipRect.height / 2 }
              : dir === "right"
              ? { left: parentRect.left + parentRect.width + TOOLTIP_WINDOW_EDGE_SPACE, top: parentRect.top + parentRect.height / 2 - tipRect.height / 2 }
              : dir === "top"
              ? { left: parentRect.left + parentRect.width / 2 - tipRect.width / 2, top: parentRect.top - tipRect.height - TOOLTIP_WINDOW_EDGE_SPACE }
  ...
  (function () {
    ...
    function MdAutocompleteCtrl($scope, $element, $mdUtil, $mdConstant, $timeout, $mdTheming, $window, $animate, $rootElement, $attrs) {
      //-- private variables
      var ctrl = this,
      ...
      function selectedItemChange(selectedItem, previousSelectedItem) {
        ...
        if (angular.isFunction($scope.itemChange) && selectedItem !== previousSelectedItem) $scope.itemChange(getItemScope(selectedItem));
      }
      ...
      function handleSearchText(searchText, previousSearchText) {
        ...
        if (angular.isFunction($scope.textChange) && searchText !== previousSearchText) $scope.textChange(getItemScope($scope.selectedItem));
  ...
  (function () {
    ...
    angular.module("material.components.autocomplete").directive("mdAutocompleteParentScope", MdAutocompleteParentScope);
    ...
    function MdChips($mdTheming, $mdUtil, $compile, $log, $timeout) {
      ...
      function compile(element, attr) {
        ...
        var chipRemoveTemplate = getTemplateByQuery("md-chips>*[md-chip-remove]") || CHIP_REMOVE_TEMPLATE,
          ...
          chipInputTemplate = getTemplateByQuery("md-chips>md-autocomplete") || getTemplateByQuery("md-chips>input") || CHIP_INPUT_TEMPLATE,
    ...
    function MdTab() {
      return {
        ...
        template: function (element, attr) {
          var label = getLabel(),
            body = getTemplate();
          return "" + "<md-tab-label>" + label + "</md-tab-label>" + "<md-tab-body>" + body + "</md-tab-body>";
          function getLabel() {
  ...
  (function () {
     ...
     * @ngInject
     */
    function MdTabsController($scope, $element, $window, $timeout, $mdConstant, $mdTabInkRipple, $mdUtil, $animate, $attrs, $compile, $mdTheming) {
      ...
      function updateHeightFromContent() {
        ...
        locked = true;
        $animate.animate($element, { height: currentHeight + "px" }, { height: newHeight + "px" }).then(function () {
          $element.css("height", "");
          locked = false;
        });
      }
